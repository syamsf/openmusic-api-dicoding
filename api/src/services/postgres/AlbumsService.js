...
class AlbumsService {
  ...
  async getAlbumById(id) {
    const query = {
      text: `SELECT 
        t1.*, 
        t2.id as song_id, 
        t2.title as song_title, 
        t2.performer as song_performer 
      FROM albums t1 LEFT JOIN songs t2 ON t1.id=t2.album_id
      WHERE t1.id = $1`,
      values: [id],
    };
  ...
  async updateAlbumCoverById(id, filename) {
    const updatedAt = new Date().toISOString();
    const query = {
      text: 'UPDATE albums SET cover_filename = $1, updated_at = $2 WHERE id = $3 RETURNING id',
    ...
    if (!result.rowCount) {
      throw new NotFoundError('Failed to update album cover. Id is not found.');
    }
  }
}
